import {createFeedbackClient,createSharedFeedbackClient} from './transport.js';
...
document.addEventListener('dayline:submit-adjustments',async event=>{
  ...
  sending=true;send.disabled=true;show('Enviando seus pedidos…');
  try{
    ...
    if(mode==='shared'){
      ...
      const previous=submissionState.get(selectedPreview.preview_id)||{};
      const submissionId=previous.payload===payload?previous.id:crypto.randomUUID();
      submissionState.set(selectedPreview.preview_id,{payload,id:submissionId});
      await sharedClient.submit(selectedPreview.preview_id,JSON.parse(payload),submissionId);
      selectedPreview.awaiting_client_feedback=false;renderMembers();
      show(`Pedidos enviados para ${selectedPreview.display_name}! O feedback ficou vinculado somente a esta prévia.`);
    }else{
